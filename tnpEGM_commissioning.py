...
def loopTree(sample, isMC):
	...
	for p in sample.path:
		...
		tree.Add(p)
		friendTreeName='weights_2017_runB'
		tree.AddFriend(friendTreeName,"/eos/cms/store/group/phys_egamma/swmukher/UL2017/PU_hs/DY_1j_madgraph_ele.pu.puTree.root")
	...
	for reg in region:
		...
		histList['el_relphoIso_puSub_ForTotEv_%s' %(reg)] = rt.TH1F('el_relphoIso_puSub_ForTotEv_%s' %(reg),'Rel. PU subtracted Photon isolation',20,0,1000)
	...
	#for ev in range(10):
		...
		if(forIso):
			...
			histList['el_relneuIso_puSub_%s' %(reg)].Fill(ne_iso_pusub/el_pt,totWeight)
                        histList['el_relphoIso_puSub_ForTotEv_%s' %(reg)].Fill(pho_iso_pusub/el_pt,totWeight)
...
for epoch in  epochs:
	...
	histData,xTitle = loopTree(dataSamples2017UL[epoch],0)
	histMC,xTitle   = loopTree(mcSamples2017UL[epoch],1)
	#nE = len(histData)
